...
| Active List | [screens/active-list.html](screens/active-list.html) | Active List with session progress bar, unchecked items at top and checked-off items below a divider, circle checkboxes; includes empty-state section (UI_DESIGN_SYSTEM §4.2) |
| Shopping Bubbles | [screens/shopping-bubbles.html](screens/shopping-bubbles.html) | Empty Master List showing Smart Bubble context suggestions (Grocery, Pharma, Camping) and the "Keep Empty" dashed bubble; PRD §7.3 |
...
| Urgent Tasks | [screens/urgent-tasks.html](screens/urgent-tasks.html) | Urgent Tasks virtual sub-hub: flat combined list of urgent task rows (urgency badge, sub-hub label, Flashlight deep-link) and overdue bill rows (bill icon, vendor, amount, red due date, Pay Now button); PRD §8.4 |
