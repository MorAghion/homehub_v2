/**
 * VouchersHub — main Vouchers Hub screen.
 *
 * Shows:
 *   - Grid of voucher sub-hub cards
...
export default function VouchersHub({
  ...
  const [showEditModal, setShowEditModal] = useState<VoucherList | null>(null)
  const [showDeleteConfirm, setShowDeleteConfirm] = useState<VoucherList | null>(null)
      ...
      <header className="bg-(--color-primary) px-4 py-4 flex items-center gap-3">
        <div className="flex-1">
          ...
          <p className="text-xs text-white/70 mt-0.5">
            {t('hub.subtitle', { count: voucherLists.length })}
          </p>
        </div>
        <button
          type="button"
                ...
                <button
                  ...
                  onClick={() => {
                    if (isEditMode) {
                      ...
                      navigate(`/vouchers/${list.id}`)
                    }
                  }}
                  data-testid="voucher-subhub-card"
                  className={[
