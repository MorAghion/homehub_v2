/**
 * VouchersHub — Sub-Hub grid for the Vouchers hub.
 *
 * Shows a responsive grid of Sub-Hub cards (e.g. "BuyMe", "Gift Cards").
...
export default function VouchersHub({ onSelectSubHub }: VouchersHubProps) {
  ...
  const { subHubs, isLoading, createSubHub, deleteSubHubs } = useVouchersHub()
      ...
      <div className="relative min-h-full">
          ...
          <HubGrid>
            {subHubs.map((subHub) => (
              <SubHubCard
                ...
                onLongPress={() => {}} // enters edit mode via useEditModeState
              />
            ))}
          </HubGrid>
        )}
        ...
        {!isEditMode && (
          <button
            type="button"
            onClick={() => setIsCreateOpen(true)}
            aria-label={t('vouchers:subHub.new')}
            className={[
              'fixed bottom-[calc(64px+16px)] end-4 w-13 h-13',
        ...
        >
          <input
            ...
            placeholder={t('vouchers:subHub.namePlaceholder')}
            maxLength={80}
            autoFocus
        ...
        {/* Bulk delete confirm — inline in edit mode toolbar */}
        {isDeleting && (
          <div
            className="fixed inset-0 z-[--z-overlay] flex items-center justify-center bg-black/40"
            aria-live="polite"
          >
